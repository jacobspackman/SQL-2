...
module.exports = {
    ...
    getPendingAppointments: (req, res) => {
        sequelize.query(`SELECT * FROM cc_appointments WHERE approved = false ORDER BY date`).then(dbRes => res.status(200).send(dbRes[0]))
    },
    ...
    completeAppointment: (req, res) => {
        ...
        sequelize.query(`UPDATE cc_appointments SET completed = true WHERE appt_id = ${apptId}`).then(dbRes => res.status(200).send(dbRes[0]))
    }
}
